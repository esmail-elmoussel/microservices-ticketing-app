...
describe("Edit ticket", () => {
  ...
  it("Should fail due to un-authorized user", async () => {
    await request(app).put("/api/orders").expect(401);
  });
  ...
  it("Should fail due to wrong user", async () => {
    ...
    const updateTicketResponse = await request(app)
      .put("/api/orders")
      .send({
        ticketId: createOrderResponse.body.id,
        title: "Asdas",
      })
      .set("Cookie", global.register());
  ...
  it("Should update title and price", async () => {
    ...
    expect(createOrderResponse.status).toBe(201);
    expect(createOrderResponse.body.title).toBe(ticket.title);
    expect(createOrderResponse.body.price).toBe(ticket.price);
